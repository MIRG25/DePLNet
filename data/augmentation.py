...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if label.shape[0] <= self.output_size[0] or label.shape[1] <= self.output_size[1] or label.shape[2] <= \
            ...
            image = numpy.pad(image, [(0, 0), (pw, pw), (ph, ph), (pd, pd)], mode='constant', constant_values=0)
            label = numpy.pad(label, [(pw, pw), (ph, ph), (pd, pd)], mode='constant', constant_values=0)

        (c, w, h, d) = image.shape
        # print("image.shape_2", image.shape)
...
class RandomRotFlip(object):
    ...
    def __call__(self, sample):
        ...
        axis = numpy.random.randint(1, 3)
        image = numpy.flip(image, axis=axis).copy()
        label = numpy.flip(label, axis=axis-1).copy()
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        cons_image = cons_image.reshape(cons_image.shape[0], cons_image.shape[1], cons_image.shape[2], cons_image.shape[3])
        cons_label = sample['cons_label']
        # print(sample['normal_range_x'],sample['normal_range_y'],sample['normal_range_z'])
        # print(sample['cons_range_x'],sample['cons_range_y'],sample['cons_range_z'])
        # print(isinstance(sample['normal_range_x'], np.ndarray))
        # print(isinstance(sample['normal_range_y'], np.ndarray))
        # print(isinstance(sample['normal_range_z'], np.ndarray))
        # print(isinstance(sample['cons_range_x'], np.ndarray))
        # print(isinstance(sample['cons_range_y'], np.ndarray))
        # print(isinstance(sample['cons_range_z'], np.ndarray))
        return {'image': torch.tensor(image), 'label': torch.tensor(label).long(),
                'x_range': torch.tensor(sample['normal_range_x']), 'y_range': torch.tensor(sample['normal_range_y']),
                'z_range': torch.tensor(sample['normal_range_z'])}, \
               {'image': torch.tensor(cons_image), 'label': torch.tensor(cons_label).long(),
                'x_range': torch.tensor(sample['cons_range_x']), 'y_range': torch.tensor(sample['cons_range_y']), 'z_range': torch.tensor(sample['cons_range_z'])}
